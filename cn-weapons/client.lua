...
RegisterNetEvent('cn-weapons:destroyGroundWeapon')
AddEventHandler('cn-weapons:destroyGroundWeapon', function()
	DeleteEntity(weapon)
end)

RegisterNetEvent('cn-weapons:allowDrop')
AddEventHandler('cn-weapons:allowDrop', function()
	canDrop = true
end)

RegisterNetEvent('cn-weapons:dropweapon')
AddEventHandler('cn-weapons:dropweapon', function(AmountOfCurrentWeapon,weaponModel,rowID)
    ...
    canDrop = false
	TriggerServerEvent("cn-weapons:dropweapon",{pos.x,pos.y,pos.z+0.75},name,rowID)
...
RegisterNetEvent('cn-weapons:doHandOver')
AddEventHandler('cn-weapons:doHandOver', function(weapon)
  --GiveWeaponToPed(PlayerPedId(), tonumber(weapon), 0, 0, false)
  TriggerEvent("attachWeapons")
end)

RegisterNetEvent('cn-weapons:dropweaponSearch')
AddEventHandler('cn-weapons:dropweaponSearch', function(weapon)
	--RemoveWeaponFromPed(PlayerPedId(),tonumber(weapon))
  TriggerEvent("attachWeapons")
end)


RegisterNetEvent('cn-weapons:giveweapon')
AddEventHandler('cn-weapons:giveweapon', function(toPlayer,AmountOfCurrentWeapon,weaponModel,rowID)
	...
	if not IsPedInAnyVehicle(PlayerPedId(),true) then
		...
		if (playing ~= false) then
			...
			TriggerServerEvent("cn-weapons:giveweapon", toPlayer, name,rowID)
			Wait(1300)
			TriggerServerEvent("weaponshop:removeSingleWeapon",rowID)
			TriggerEvent("attachWeapons")
		else
...
Citizen.CreateThread(function()
    while true do
        ...
        if invehicle == nil then
        	...
        	TriggerEvent("attachWeapons")
			TriggerEvent("AnimSet:Set")
			TriggerEvent("carhud:enterVehicleSetTurbulence",invehicle)
        end
        ...
        if not invehicle and IsPedInAnyVehicle(PlayerPedId(), false) then
			...
			local vehicleModel = GetEntityModel(GetVehiclePedIsUsing(PlayerPedId()))
        	if vehicleModel == `taxi` then
        		TriggerEvent("taximeter:EnteredTaxi")
        	end

			if IsThisModelAPlane(vehicleModel) or IsThisModelAHeli(vehicleModel) then
				TriggerEvent("carhud:enterVehicleSetTurbulence",true)
			end
...
AddEventHandler('attachWeapons', function()
	...
	local currentWeapons = exports["cn-inventory"]:GetCurrentWeapons()
	weapcount = 1
	local curw = GetSelectedPedWeapon(PlayerPedId())
