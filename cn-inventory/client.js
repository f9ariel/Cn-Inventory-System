...
//  $.post("http://cn-inventory/SlotJustUsed", JSON.stringify({target: targetSlot, origin: originSlot, itemid: itemidsent }));
let recentused = [];
...
on('inventory-open-request', async () => {
    ...
    const currentTarget = exports['cn-target'].GetCurrentEntity()
...
on('cn-items:SetAmmo', (sentammoTable) => {
    if (sentammoTable.length > 0 ){
        if (sentammoTable) {
