...
onNet("server-update-item", async (player, itemidsent,slot,data) => {
    ...
    let string = `UPDATE character_inventory SET information='${data}' WHERE item_id='${itemidsent}' and name='${playerinvname}' and slot='${slot}'`
...
function functionRemoveAny(player, itemidsent, amount, openedInv) {
    let src = source
    let playerinvname = 'ply-' + player
    let string = `DELETE FROM character_inventory WHERE name='${playerinvname}' and item_id='${itemidsent}' LIMIT ${amount}`
    exports.ghmattimysql.execute(string,{},function() {
        emit("server-request-update-src",player,src)
    });
}
...
onNet("server-request-update", async (player) => {
    let src = source
    let playerinvname = 'ply-' + player
    let string = `SELECT count(item_id) as amount, id, item_id, name, information, slot, dropped, quality FROM character_inventory WHERE name = 'ply-${player}' group by item_id`; 
...
onNet("inventory-degItem", async(itemID) => {
    let amount = 4000000
    exports.ghmattimysql.execute(`UPDATE character_inventory set creationDate = ${amount} WHERE id = ${itemID}`, {}, function() {});
});

RegisterServerEvent("server-request-update-src")
onNet("server-request-update-src", async (player,src) => {
    let playerinvname = 'ply-' + player
    let string = `SELECT count(item_id) as amount, item_id, id, name, information, slot, dropped, creationDate, MIN(creationDate) as creationDate, quality FROM character_inventory WHERE name = '${playerinvname}' group by item_id`; // slot
    exports.ghmattimysql.execute(string, {}, function(inventory) {
    emitNet("inventory-update-player", src, [inventory,0,playerinvname]);
    });
});
    ...
    return new Promise((resolve, reject) => {
        ...
        if (!isNaN(itemid)) {
            ...
            switch(itemid.toLowerCase()) {
                case "idcard":
                    if(itemdata == itemdata.fake){
                        returnInfo = JSON.stringify({
                            ...
                            FirstName: itemdata.first.replace(/[^\w\s]/gi, ''),
                            LastName: itemdata.last.replace(/[^\w\s]/gi, ''),
                            Sex: itemdata.sex,
                            DOB: itemdata.dob 
                        })
                        timeout = 1
                        ...
                        exports.ghmattimysql.execute(string,{}, function(result) {
                            ...
                            let ts = new Date(parseInt(result[0].dob))
                            let testDate = ts.getFullYear() + "-" + ("0"+(ts.getMonth()+1)).slice(-2) + "-" + ("0" + ts.getDate()).slice(-2)
                            returnInfo = JSON.stringify({
                                CitizenId: player.toString(),
                                FirstName: result[0].first_name.replace(/[^\w\s]/gi, ''),
                                LastName: result[0].last_name.replace(/[^\w\s]/gi, ''),
                                Sex: gender,
                                // DOB: result[0].dob
                                DOB: testDate
                            })
                ...
                case "drivingtest":
                    if (data.id) {
                        let string = `SELECT * FROM character_driving_tests WHERE id = '${data.id}'`;
                        exports.ghmattimysql.execute(string, {}, function(result) {
                            if (result[0]) {
                                let ts = new Date(parseInt(result[0].createdate) * 1000)
                                let testDate = ts.getFullYear() + "-" + ("0"+(ts.getMonth()+1)).slice(-2) + "-" + ("0" + ts.getDate()).slice(-2)
                                returninfo = JSON.stringify({ ID: result[0].id, CID: result[0].cid, Instructor: result[0].instructor, Date: testDate })
                                timeout = 1;
                                clearTimeout(timeout)
                            }
                            return resolve(returninfo);
...
onNet("server-inventory-give", async (player, itemid, slot, amount, generateInformation, itemdata, openedInv) => {
    ...
    let query = `INSERT INTO character_inventory (name,item_id,information,slot,creationDate) VALUES ${values};`
...
onNet("server-inventory-refresh", async (player, sauce) => {
    ...
    let string = `SELECT count(item_id) as amount, id, name, item_id, information, slot, dropped, creationDate, quality FROM character_inventory where name= 'ply-${player}' group by slot`;
    exports.ghmattimysql.execute(string,{}, function(inventory) {
        if (!inventory){}else{
...
onNet("server-inventory-open", async ( coords, player, secondInventory, targetName, itemToDropArray, sauce) => {
    ...
    exports.ghmattimysql.execute(string,{}, function(inventory) {
        ...
        // console.log(secondInventory, targetName)
        emitNet('current-items', src, invArray)
        if(secondInventory == "1") {
            var targetinvname = targetName
            let string = `SELECT count(item_id) as amount, id, name, item_id, information, slot, dropped, creationDate, quality FROM character_inventory WHERE name = '${targetinvname}' group by slot`;
        ...
        {   
            ...
            for (let Key in itemToDropArray) {
                for (let i = 0; i < itemToDropArray[Key].length; i++) {
                    ...
                    db(`UPDATE character_inventory SET slot='${i+1}', name='${NewDroppedName}', dropped='1' WHERE name='${Key}' and slot='${objectToDrop.faultySlot}' and item_id='${objectToDrop.faultyItem}' `);
                }
            }
...
// function sendClientItemList(pId) {
...
onNet("server-inventory-close", async (player, targetInventoryName) => {
    ...
    if (targetInventoryName.indexOf("Drop") > -1 && DroppedInventories[targetInventoryName]) {
        if (DroppedInventories[targetInventoryName].used === false ) {
            ...
            let string = `SELECT count(item_id) as amount, item_id, name, information, slot, dropped, quality FROM character_inventory WHERE name='${targetInventoryName}' group by item_id `;
            exports.ghmattimysql.execute(string,{}, function(result) {
...
onNet("SniffCID", async (cid,src) => {
    let name = cid;
    let string = `SELECT count(item_id) as amount, id, item_id, name, information, slot, lity, dropped, quality FROM character_inventory WHERE name='${name}' and item_id IN (${IllegalSearchString}) group by item_id`;
...
onNet("sniffLicensePlateCheck", async (plate) => {
    ...
    let string = `SELECT count(item_id) as amount, id, item_id, name, information, slot, quality, dropped, quality FROM character_inventory WHERE name='${car}' OR name='${glovebox}') and item_id IN (${IllegalSearchString}) group by item_id`;
...
onNet("inv:delete", async (inv) => {
    db(`DELECT FROM character_inventory WHERE name='${inv}'`);
});
...
onNet("server-inventory-remove-slot", async (player, itemidsent,amount,slot) => {
    ...
    db(`DELETE FROM character_inventory WHERE name='${playerinvname}' and item_id='${itemidsent}' and slot='${slot}' LIMIT ${amount}`);
});
...
onNet("server-ragdoll-items", async (player) => {
    ...
    db(`UPDATE character_inventory SET name='${newInventoryName}', WHERE name='${currInventoryName}' and dropped=0 and item_id="mobilephone" `);
    db(`UPDATE character_inventory SET name='${newInventoryName}', WHERE name='${currInventoryName}' and dropped=0 and item_id="idcard" `);
    await db(`DELETE FROM character_inventory WHERE name='${currInventoryName}'`);
    db(`UPDATE character_inventory SET name='${currInventoryName}', WHERE name='${newInventoryName}' and dropped=0`);
});
...
onNet("server-inventory-move", async (player, data, coords) => {
    // console.log("server-inventory-move")
    let src = source
    ...
    if (purchase) {
        ...
        if (!PlayerStore) {
            for (let i = 0; i < parseInt(amount); i++) {
                ...
                db(`INSERT INTO character_inventory (item_id, name, information, slot, creationDate) VALUES ('${itemidsent}','${targetName}','${info}','${targetslot}','${creationDate}' );`);
            }
        } else if (crafting) {
            ...
            if (targetName.indexOf("Drop") > -1 || targetName.indexOf("hidden") > -1) {
                db(`INSERT INTO character_inventory SET slot='${targetslot}', name='${targetName}', dropped='1' WHERE slot='${startslot}' AND name='${startname}'`);
            } else {
...
onNet("server-inventory-stack", async (player, data, coords) => {
    // console.log("server-inventory-stack")
    let targetslot = data[0]
    ...
    if (purchase) {
        ...
        let string = `SELECT item_id, id FROM character_inventory WHERE slot='${originSlot}' and name='${originInventory}' LIMIT ${moveAmount}`;
        exports.ghmattimysql.execute(string,{}, function(result) {
            var itemids = "0"
...
onNet('imtoohot', (dataname) => {
    db(`DELETE FROM character_inventory WHERE name='${dataname}'`)
});
...
onNet("server-inventory-swap", (player, data, coords) => {
    ...
    let string = `SELECT id FROM character_inventory WHERE slot='${targetslot}' AND name='${targetname}'`;
    exports.ghmattimysql.execute(string,{}, function(startid) {
        ...
        let string = false;
        if (targetname.indexOf("Drop") > -1 || targetname.indexOf("hidden") > -1) {
            string = `UPDATE character_inventory SET slot='${targetslot}', name ='${targetname}', dropped='1' WHERE slot='${startslot}' AND name='${startname}'`;
        } else {
            string = `UPDATE character_inventory SET slot='${targetslot}', name ='${targetname}', dropped='0' WHERE slot='${startslot}' AND name='${startname}'`;
        }
        exports.ghmattimysql.execute(string,{}, function(inventory) {
...
onNet('onResourceStart', (resource) => {
    if (resource == GetCurrentResourceName()){
        db(`DELETE FROM character_inventory WHERE name like '%Drop%'`)
    }
})

function deleteHidden() {
    db(`DELETE FROM character_inventory WHERE name like '%Hidden%' OR name like '%trash%'`)
}
    ...
    for (let key in storeList) {
        if (storeList[key].house_model == 4) {
            ...
            if (amount > 0) {
                ...
                let string = `SELECT item_id FROM character_inventory WHERE name = '${name}' and slot = '${slot}'`;
                exports.ghmattimysql.execute(string,{}, function(inventory) {
                    if (inventory.length > 0) {
                        ...
                        let string = `DELETE FROM character_inventory WHERE name='${name}' and slot = '${slot}' LIMIT ${amount}`;
                        exports.ghmattimysql.execute(string,{}, function() {});
